...
import lib.yuncorelot.body.WifiRadioVapUsers;
	 					...
	 					this.handleData("0","0",yuncorelotDataBody,yuncorelotData.getNow(),time,true);
   		 				...
   		 				if(registerInfo.get("bind_status").toString().equals("1") || registerInfo.get("bind_status").toString().equals("2")){
   		 					...
   		 					if(yuncorelotDataBody.getSystem() != null){
   		 						String devPassword = registerInfo.get("weblogin_pwd").toString();
   		 						if(registerInfo.get("pwd_encrypt") != null){
	   		 						String pwdEncrypt = registerInfo.get("pwd_encrypt").toString();
			   		 				if(pwdEncrypt.equals("2")){
			   		 					devPassword = Helper.encrypt(devPassword.toString(),"MD5");
			   		 				}else if(pwdEncrypt.equals("3")){
			   		 					devPassword = Helper.encrypt(devPassword,"SHA-256");
			   		 				}
   		 						}
				
   		 						String command = "{\"type\":\"bind\",\"bind\":\""+registerInfo.get("bind").toString()+"\",\"token\":\""+Helper.generateDevToken(registerInfo.get("dev_username").toString(),devPassword,registerInfo.get("mac").toString(), time)+"\"";
...
/*	    		if(!onlySysFlag && data.getSystem() != null && (oldStatus == null || oldStatus.equals(deviceConf.STATUS_OFFLINE))){
    							...
    							for(int m=0;m<wifiRadio.getVap().size();m++){
    								...
    								for(int n=0;n<baseWifiRadio.getVap().size();n++){
    									...
    									if(baseWifiRadioVap.getId().equals(wifiRadioVap.getId())){//找到vap
    										String[] vapProperty = {"Id","Enable","Vlan_id","Bssid","Ssid","Ssid_hide","Encode","Password","Radius_key","Radius_server","Radius_port","Radius_acct_key","Radius_acct_server","Radius_acct_port","Radius_nai_realm","Radius_domain","Radius_network_auth_type","Radius_venue_name"};
    ...
    private boolean handleAllChildSystem(List<Ichild> list,String toUid,String gid,Long time,Map<String,Object> pdevice,String now) throws Exception{
    	...
    	try{
    		...
    		for(int i=0;i<list.size();i++){
    			...
    			if(childSystem != null){
    	    		...
    	    		String newversion = "";
    	    		String package_fid = "";
    	    		String sn = childSystem.getSn() != null ? childSystem.getSn() : "";
    	    		String is_tdma = childSystem.getIs_tdma() != null ? childSystem.getIs_tdma() : "0";
    	    		Map<String,Object> newPackage = this.redisCache.getCloudLatestPackageByType(type,this.getJedisResourse());
    				if(newPackage == null){
    				...
    				addPstmt.setObject(23,sn);
    				addPstmt.setObject(24,is_tdma);
    				addPstmt.addBatch();
...
/*    private List<Map<String,String>> mergeList(List<Map<String,String>> bases,List<Map<String,String>> datas,String keyword){
    ...
    private Map<String,String> getDeviceAddData(String mac,String pmac,String toUid,String gid,lib.yuncorelot.body.System systemData,Long time){
		...
		if(systemData.getSn() == null){
			result.put("sn","");
		}else{
			result.put("sn",systemData.getSn());
		}
		...
		if(systemData.getSn() != null){
			result.put("is_tdma",systemData.getIs_tdma());
		}
		this.redisCache.delete(redisConf.REGISTER+this.mac,this.getJedisResourse());
    	return result;
    }
    ...
    private Map<String,String> getDeviceUpdateData(Map<String,Object>device,String mac,String pmac,String toUid,String gid,lib.yuncorelot.body.System systemData,Long time){
    	...
    	if(systemData.getSn() != null){
    		if(device.get("sn") ==  null || this.isDifferentNoNull(systemData.getSn(), device.get("sn").toString())){
    			result.put("sn", systemData.getSn());
    		}
    	}
    	...
    	if(systemData.getIs_tdma() != null){
    		if(device.get("is_tdma") ==  null || this.isDifferentNoNull(systemData.getIs_tdma(), device.get("is_tdma").toString())){
    			result.put("is_tdma", systemData.getIs_tdma());
    		}
    	}
