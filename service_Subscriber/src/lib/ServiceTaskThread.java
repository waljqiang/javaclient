...
public class ServiceTaskThread extends Thread{

	private static final Log log = LogFactory.getLog(ServiceTaskThread.class);
	...
	public void run() {
		...
		try {
			...
			//监测队列长度
			ListenerQueueThread listenerQueueThr = new ListenerQueueThread();
			listenerQueueThr.setName("listener-queue");
			listenerQueueThr.start();
